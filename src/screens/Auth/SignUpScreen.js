        ...
        <Formik
          ...
          {({ handleChange, handleBlur, handleSubmit, values, errors, isValid }) => (
            <>
              ...
              />
              {errors.name && <Text style={{ color: '#f00', fontSize: 14 }}>{errors.name}</Text>}
              <TextInput
                name="phone"
                placeholder="Telefon Numaranız (5321234567)"
                style={{ height: 50, width: '90%', padding: 10, margin: 10, borderColor: '#000', borderWidth: 1, borderRadius: 10, fontSize: 16 }}
              ...
              />
              {errors.phone && <Text style={{ color: '#f00', fontSize: 14 }}>{errors.phone}</Text>}
              <TextInput
                name="email"
              ...
              <View
                ...
                <TouchableOpacity onPress={() => setIsSecurePass(!isSecurePass)}>
                  <Icon style={{ marginRight: 10 }} size={20} name={isSecurePass ? 'eye-slash' : 'eye'} color="#aaa" />
                </TouchableOpacity>
              </View>
              ...
              <View
                ...
                <TouchableOpacity onPress={() => setIsSecurePassConfirm(!isSecurePassConfirm)}>
                  <Icon style={{ marginRight: 10 }} size={20} name={isSecurePassConfirm ? 'eye-slash' : 'eye'} color="#aaa" />
                </TouchableOpacity>
              </View>
              {errors.passwordConfirm && <Text style={{ color: '#f00', fontSize: 14 }}>{errors.passwordConfirm}</Text>}
              <View style={{ width: '50%' }}>
                <Button color="#f00" onPress={handleSubmit} disabled={!isValid} title="Kaydet" />
              </View>
            </>
          )}
        </Formik>
