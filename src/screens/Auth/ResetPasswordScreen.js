  ...
  return (
    <SafeAreaView style={{ flex: 1, alignItems: 'center', justifyContent: 'center' }}>
      <View style={{ width: '80%', alignItems: 'center', padding: 10, backgroundColor: '#ddd', borderRadius: 30 }}>
        ...
        <Formik validationSchema={loginValidationSchema} initialValues={{ email: '' }} onSubmit={values => resetPassword(values.email)}>
          {({ handleChange, handleBlur, handleSubmit, values, errors, isValid }) => (
            <>
              ...
              <View style={{ width: '50%' }}>
                <Button color="#f00" onPress={handleSubmit} disabled={!isValid} title="Sıfırla" />
              </View>
            </>
          )}
        </Formik>
